...
def load_data_folder(foldername,my_mesh,stride_read,index, init = 0, end = 10e6, index_to_read=None):
    """
    ...
    for file in os.listdir(foldername):
        if file.endswith(".af"):
            list_files.append((foldername+file,float((file.split(".")[0]).split("_")[2])))
    ...
    return np.array(data), np.array(zs)*my_mesh.dt
